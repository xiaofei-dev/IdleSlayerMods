...
def register_fonts() -> tuple[str, str, str, str]:
    ...
    pdfmetrics.registerFont(TTFont("ProgressionSans", str(regular)))
    pdfmetrics.registerFont(TTFont("ProgressionSans-Bold", str(bold if bold.exists() else regular)))
    pdfmetrics.registerFont(TTFont("ProgressionSans-Italic", str(italic if italic.exists() else regular)))
    mono_name = "Courier"
    if mono.exists():
        mono_name = "ProgressionMono"
        pdfmetrics.registerFont(TTFont(mono_name, str(mono)))
    return "ProgressionSans", "ProgressionSans-Bold", "ProgressionSans-Italic", mono_name
...
class Template(BaseDocTemplate):
    def __init__(self, target: Path, document_name: str):
        ...
        super().__init__(str(target), pagesize=A4, leftMargin=22*mm, rightMargin=22*mm,
            ...
            subject="AutoProgression documentation for Idle Slayer")
    ...
    def decorate(self, canvas, doc):
        ...
        canvas.drawString(22*mm, PAGE_HEIGHT-10*mm, "AutoProgression")
...
def cover(title: str, subtitle: str):
    ...
    flow.extend([
        Paragraph("AutoProgression", STYLES["cover_title"]),
...
def build_user_guide() -> Path:
    target = OUTPUT / f"AutoProgression-User-Guide-{VERSION}.pdf"
...
def build_manual() -> Path:
    target = OUTPUT / f"AutoProgression-Complete-Manual-{VERSION}.pdf"
    story = cover("Complete Manual",
        "Ascension, purchases, quests, craftables, minions, resources, configuration, and troubleshooting")
...
def main():
    ...
    package = OUTPUT / f"AutoProgression-Documentation-{VERSION}.zip"
